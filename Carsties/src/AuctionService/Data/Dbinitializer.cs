...
namespace AuctionService.Data
{
    public class Dbinitializer
    {
        ...
        {
            // Create a scope of services we are using here and after we finished we dispose of them (that is we use the 'using' statement)
        ...
        {
            // If there are any pending migrations, apply them. Create the database if it doesn't exist.
            context.Database.Migrate();

            // If there are any auctions, we assume the DB has been seeded and we return without doing anything.
            if (context.Auctions.Any())
            {
                Console.WriteLine("DB has data. No need for seeding 🙂");
            ...
            // Keep the auctions we created in memory until we have added all of them to the database
            context.AddRange(auctions);

            // Save the auctions from memory to the database
            context.SaveChanges();
        }
    }
}